...
SFML_BulletSpriteObject* SFML_PlayableSpriteObject::processEvents(float elapsedTime, sf::Transform transform, sf::Vector2f mouseScreenPosition)
{
	...
	if (leftButtonPressed || rightButtonPressed)
	{
		...
		sf::Vector2f charScreenPos = getObjectScreenPosition(transform);
		...
		if (rightButtonPressed)
		{
			...
			{				
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::LShift))
				{
					...
					bullet = shoot();
					if (getCurrentAnimation()->animationCompleted())
					{
	...
	else
	{
		...
		{
			m_readyToShoot = true;
			m_timeSinceLastShoot -= m_rangeWeaponCooldown;
		}
	}
...
int	SFML_PlayableSpriteObject::setFlashlightMeleeAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_flashlightMeleeAnimationID]->setInterrupt(false);
...
int	SFML_PlayableSpriteObject::setKnifeMeleeAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_knifeMeleeAnimationID]->setInterrupt(false);
...
int	SFML_PlayableSpriteObject::setPistolMeleeAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_pistolMeleeAnimationID]->setInterrupt(false);
...
int	SFML_PlayableSpriteObject::setPistolShootAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_pistolShootAnimationID]->setInterrupt(false);
...
int SFML_PlayableSpriteObject::setPistolReloadAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_pistolReloadAnimationID]->setInterrupt(false);
...
int	SFML_PlayableSpriteObject::setRifleMeleeAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_rifleMeleeAnimationID]->setInterrupt(false);
...
int	SFML_PlayableSpriteObject::setRifleShootAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_rifleShootAnimationID]->setInterrupt(false);
...
int SFML_PlayableSpriteObject::setRifleReloadAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	...
	m_animatedSpriteList[m_rifleReloadAnimationID]->setInterrupt(false);
...
void SFML_PlayableSpriteObject::equipRifle()
{
	...
	m_rangeWeaponCooldown = 0.01f;
}

bool SFML_PlayableSpriteObject::toIdleState()
{

	if (m_currentAnimation->canBeInterrupted() || m_currentAnimation->animationCompleted())
	{
...
void SFML_PlayableSpriteObject::changeWeapon()
{
	...
	{
		if (m_equippedWeapon != FLASHLIGHT)
		{
			equipFlashlight();
		}
	}
...
SFML_BulletSpriteObject* SFML_PlayableSpriteObject::shoot()
{
	...
	if (toShootState())
	{
		if (m_readyToShoot)
		{
			...
			bulletStart.x = cosf(getRotation());
			bulletStart.y = sinf(getRotation()) + 32;


			SFML_BulletSpriteObject*	bullet = new SFML_BulletSpriteObject();
			bullet->applyTexture(&m_bulletTexture);
			bullet->setPosition(getObjectWorldPosition() + bulletStart);
