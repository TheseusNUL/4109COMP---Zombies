...
void SFML_BulletSpriteObject::applyTexture(sf::Texture* texture)
{
	...
	m_spriteOriginPoint = sf::Vector2f(m_spriteTexture->getSize().x / 2, m_spriteTexture->getSize().y / 2);
	m_textureRect = sf::IntRect(0, 0, m_spriteTexture->getSize().x, m_spriteTexture->getSize().y);
}
